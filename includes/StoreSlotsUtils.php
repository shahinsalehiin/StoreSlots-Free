    ...
    class StoreSlotsUtils
    {
        ...
        public function save_settings($data = array()) {
            ...
            if (!empty($data)) {
                ...
                $this->storeslots_generate_delivery_dates( $data );
                $this->storeslots_generate_pickup_dates( $data );
                $this->storeslots_generate_delivery_times( $data );
                $this->storeslots_generate_pickup_times( $data );
        ...
        /**
         * define generate delivery dates
         * @param $settings_data[]
         */
        public function storeslots_generate_delivery_dates ( $settings_data = []) {

            $delivery_available_days = !empty($settings_data) && !empty($settings_data['ssd_available_days']) ? $settings_data['ssd_available_days'] : '';
            $delivery_days = !empty($settings_data) && !empty($settings_data['storeslots_delivery_days']) ? explode(',', $settings_data['storeslots_delivery_days']) : array();

            $delivery_dates = [];
            $i = 0;

            if( $delivery_available_days > 0 && !empty( $delivery_days ) ){
                while (1) {
                    $next_date = date ("Y-m-d", strtotime ("+$i day"));
                    $nameOfDay = strtolower (date ('l', strtotime ($next_date)));
                    if (in_array ($nameOfDay, $delivery_days)) {
                        $delivery_dates[] = $next_date;
                    }
                    if(count($delivery_dates) == $delivery_available_days){
                        break;
                    }
                    $i++;
                }
            ...
            update_option('storeslot_delivery_dates', $delivery_dates);

        }

        /**
         * define generate pickup dates
         * @param $settings_data[]
         */
        public function storeslots_generate_pickup_dates ($settings_data = []) {

            $pickup_available_days = !empty($settings_data) && !empty($settings_data['ssp_available_days']) ? $settings_data['ssp_available_days'] : 0;
            $pickup_days = !empty($settings_data) && !empty($settings_data['storeslots_pickup_days']) ? explode (',', $settings_data['storeslots_pickup_days']) : array();

            $pickup_dates = [];
            $i = 0;
            if( $pickup_available_days > 0 && !empty( $pickup_days ) ){
                while (1) {
                    $next_date = date ("Y-m-d", strtotime ("+$i day"));
                    $nameOfDay = strtolower (date ('l', strtotime ($next_date)));
                    if (in_array ($nameOfDay, $pickup_days)) {
                        $pickup_dates[] = $next_date;
                    }
                    if(count($pickup_dates) == $pickup_available_days){
                        break;
                    }
                    $i++;
                }
            ...
            update_option('storeslot_pickup_dates', $pickup_dates);

        }

        /**
         * define generate delivery times
         * @param $settings_data[]
         */
        public function storeslots_generate_delivery_times($settings_data = []) {
            $delivery_times = array();
            $ssd_time_starts_from   = !empty($settings_data) && !empty($settings_data['ssd_time_starts_from']) ? $settings_data['ssd_time_starts_from'] : 0;
            $ssd_time_ends_from     = !empty($settings_data) && !empty($settings_data['ssd_time_ends_from']) ? $settings_data['ssd_time_ends_from'] : 0;
            $ssd_slot_duration_time = !empty($settings_data) && !empty($settings_data['ssd_slot_duration_time']) ? $settings_data['ssd_slot_duration_time'] : 0;
            $ssd_slot_duration_type = !empty($settings_data) && !empty($settings_data['ssd_slot_duration_type']) ? $settings_data['ssd_slot_duration_type'] : 'hour';
            $ssd_time_format        = !empty($settings_data) && !empty($settings_data['ssd_time_format']) ? $settings_data['ssd_time_format'] : 24;
            
            $start_time = strtotime($ssd_time_starts_from);
            $end_time   = strtotime($ssd_time_ends_from);
            $duration   = ($ssd_slot_duration_type == 'minutes') ? $ssd_slot_duration_time * 60 : ( ($ssd_slot_duration_time * 60) * 60);
            
            $deliveryTimes = array();
            if( $start_time > 0 && $end_time > 0 && $duration > 0){
                for( $i = $start_time; $i<= $end_time; $i +=$duration ) {
                    $deliveryTimes[] = ($ssd_time_format == 12) ? date("h:i A",$i) : date("H:i",$i);
                }
            }
            
            // to create delivery time slots
            if( !empty( $deliveryTimes ) ){
                $numberOfTime = count( $deliveryTimes );
                $j = 1;
                $delivery_times[] = 'Select delivery time';
                foreach( $deliveryTimes as $time ){
                    if( $j < $numberOfTime ){
                        $time_slot_key = date("H:i", strtotime( $time)) . ' - ' . date("H:i", strtotime( $deliveryTimes[$j]));
                        $time_slot_option = $time . ' - ' . $deliveryTimes[$j];
                        $delivery_times[$time_slot_key] = $time_slot_option;
                    }
                $j++;}
            }

            // update options
            update_option( "storeslots_delivery_times", $delivery_times);
         ...
         * @param $settings_data[]
         */
        public function storeslots_generate_pickup_times($settings_data = []) {
            $pickup_times = array();

            $ssp_time_starts_from   = !empty($settings_data) && !empty($settings_data['ssp_time_starts_from']) ? $settings_data['ssp_time_starts_from'] : 0;
            $ssp_time_ends_from     = !empty($settings_data) && !empty($settings_data['ssp_time_ends_from']) ? $settings_data['ssp_time_ends_from'] : 0;
            $ssp_slot_duration_time = !empty($settings_data) && !empty($settings_data['ssp_slot_duration_time']) ? $settings_data['ssp_slot_duration_time'] : 0;
            $ssp_slot_duration_type = !empty($settings_data) && !empty($settings_data['ssp_slot_duration_type']) ? $settings_data['ssp_slot_duration_type'] : 'hour';
            $ssp_time_format        = !empty($settings_data) && !empty($settings_data['ssp_time_format']) ? $settings_data['ssp_time_format'] : 24;

            $start_time = strtotime($ssp_time_starts_from);
            $end_time   = strtotime($ssp_time_ends_from);
            $duration   = ($ssp_slot_duration_type == 'minutes') ? $ssp_slot_duration_time * 60 : ( ($ssp_slot_duration_time * 60) * 60);

            $pickupTimes = array();
            if( $start_time > 0 && $end_time > 0 && $duration > 0 ){
                for( $i = $start_time; $i <=$end_time; $i +=$duration ) {
                    $pickupTimes[] = ($ssp_time_format == 12) ? date("h:i A",$i) : date("H:i",$i);
                }
            }

            // to create pickup time slots
            if( !empty( $pickupTimes ) ){
                $numberOfTime = count( $pickupTimes );
                $j = 1;
                $pickup_times[] = 'Select pickup time';
                foreach( $pickupTimes as $time ){
                    if( $j < $numberOfTime ){
                        $time_slot_key = date("H:i", strtotime( $time)) . ' - ' . date("H:i", strtotime( $pickupTimes[$j]));
                        $time_slot_option = $time . ' - ' . $pickupTimes[$j];
                        $pickup_times[$time_slot_key] = $time_slot_key;
                    }
                $j++;}
            }

            //update option
            update_option( "storeslots_pickup_times", $pickup_times);
